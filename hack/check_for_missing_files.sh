...
function count_unnest() {
  ...
  NUM_MISSING=`bq --project_id=${PROJECT_ID} --format=sparse query --dataset_id=${DATASET_NAME} --nouse_legacy_sql "${MISSING_FILE_SQL}" | tail -n+3 | tr -d "[:blank:]"`
...
# count_unnest "analysisactivity" "derived_from_file_id"
